...
public class Buscar extends JFrame {
	...
	public Buscar() {
		...
		modeloHospedes.addRow(new Object[] { "ID", "NOMBRE", "APELLIDO", "FECHA DE NACIMIENTO", "NACIONALIDAD", "TELEFONO",
		"ID RESERVA" });
		...
		JLabel lblExcluir = new JLabel("ELIMINAR");
	...
	private void cargarTablaReservas() {
		...
		reservas.forEach(reserva -> modelo.addRow(new Object[] { reserva.getId(), reserva.getDataEntrada(),
				reserva.getDataSalida(), reserva.getValor(), reserva.getFormaPago() }));
	}
	...
	private void cargarTablaHuespedes() {
		...
		huespedes.forEach(huesped -> modeloHospedes.addRow(new Object[] { huesped.getId(), huesped.getNombre(),
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" e "y"	
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
		...
		private void cargarBusqueda() {
			...
			List<Reserva> reservas = reservaController.listar(txtBuscar.getText());
			List<Huesped> huespedes = huespedController.listar(txtBuscar.getText());
		...
		private boolean noTieneFilaElegidaHuespedes() {
			return tbHospedes.getSelectedRowCount() == 0 || tbHospedes.getSelectedColumnCount() == 0;
		}
			...
			if (noTieneFilaElegidaReservas()) {
				Optional.ofNullable(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), tbHospedes.getSelectedColumn()))
						.ifPresentOrElse(fila -> {
							...
							String nombre = (String) modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 1);
							String apellido = (String) modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 2);
							Date fecha_nacimiento = Date
									.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 3).toString());
							String nacionalidad = (String) modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 4);
							String telefono = (String) modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 5);
							Integer  idReserva = Integer.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 6).toString());
							huespedController.modificar(nombre, apellido, fecha_nacimiento, nacionalidad, telefono,
									idReserva, id);
