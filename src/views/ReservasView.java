...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblValorSimbolo.setBounds(160, 332, 17, 25);
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				calcularEstadia(txtDataE,txtDataS);
			}
		});
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {		
					String fechaEntrada = ((JTextField) txtDataE.getDateEditor().getUiComponent()).getText();
					String fechaSalida = ((JTextField) txtDataS.getDateEditor().getUiComponent()).getText();
...
/*		JPanel btnsiguiente = new JPanel();
		...
		JLabel lblSeguinte = new JLabel("SIGUIENTE");
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" e "y"	
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	  ...
	  //funcion para calcular el valor de la estadía
		private void calcularEstadia (JDateChooser txtDataE, JDateChooser txtDataS) {
			if (txtDataS.getDate() != null && txtDataE.getDate() != null) {
				if (txtDataS.getDate().after(txtDataE.getDate())) {
					lblValorSimbolo.setVisible(true);
					int valorPorNoche = 600;
					long millisecondsToFechaS = txtDataS.getDate().getTime();
					long millisecondsToFechaE = txtDataE.getDate().getTime();
					...
					txtValor.setText("");
					JOptionPane.showMessageDialog(null, "La fecha de Salida no puede ser anterior a la fecha se Salida");
				}
			}
		}
	...
	@SuppressWarnings("static-access")
		private void limpiar() {
			this.txtDataE.setCalendar(null);
			this.txtDataS.setCalendar(null);
